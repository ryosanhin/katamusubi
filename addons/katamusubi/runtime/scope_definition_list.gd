...
const RandomID := preload("res://addons/katamusubi/editor/random_id.gd")
const RollbackAction := preload("res://addons/katamusubi/editor/rollback_action.gd")
...
func _get_current_id_list() -> Array[StringName]:
	...
	var tmp_list := scope_definitions.map(
			func(definition: ScopeDefinition) -> StringName:

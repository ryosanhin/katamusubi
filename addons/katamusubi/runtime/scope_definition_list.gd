...
@export var scope_definitions: Array[Katamusubi.ScopeDefinition] = []
...
func add_scope_definition(
	definition: Katamusubi.ScopeDefinition
) -> Katamusubi.Editor.RollbackAction:
	const RollbackAction := Katamusubi.Editor.RollbackAction
...
func update_scope_definition(
	...
	if definition == null:
		...
		return Katamusubi.Editor.RollbackAction.new(false, Callable())
	...
	return Katamusubi.Editor.RollbackAction.new(
			true,
			Callable(
		func() -> void:
...
func remove_scope_definition(
	...
	# 参照しているスコープがあれば参照を解除
	var modified_definitions: Array[Katamusubi.ScopeDefinition] = []
...
func get_new_id() -> StringName:
	...
	var id := Katamusubi.Editor.RandomID.get_random_id()
	var loop_count := 1
	...
	while current_id_list.has(id):
		if loop_count >= MAX_LOOP_COUNT:
			...
			return &""
		id = Katamusubi.Editor.RandomID.get_random_id()
		loop_count += 1
...
func _get_current_id_list() -> Array[StringName]:
	...
	var tmp_list := scope_definitions.map(
			func(definition: Katamusubi.ScopeDefinition) -> StringName: 
...
func get_scope_definition(scope_id: StringName) -> Katamusubi.ScopeDefinition:

...
static func _get_arguments(method_data: Dictionary) -> Array[Katamusubi.ArgumentData]:
	const KEY_NAME := "name"
	const KEY_CLASS_NAME := "class_name"
	const KEY_TYPE := "type"

	var arguments: Array[Katamusubi.ArgumentData] = []
	...
	for argument_data: Dictionary in method_data.get("args", []):
		...
		arguments.append(
				Katamusubi.ArgumentData.new(arg_name, arg_class_name, arg_variant_type)
		)
...
static func get_injection_arguments(script: Script) -> Array[Katamusubi.ArgumentData]:

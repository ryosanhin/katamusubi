...
func _build() -> bool:
	...
	# Array は Array[Katamusubi.ScopeDefinition]
	var definitions_by_scene: Dictionary[StringName, Array] = {}
	for definition in Katamusubi.DEFINITION_LIST.scope_definitions:
	...
	for scene_uid in definitions_by_scene:
		var definitions: Array[Katamusubi.ScopeDefinition] = []
		definitions.assign(definitions_by_scene[scene_uid])
		errors.append_array(Katamusubi.Editor.TscnScanner.scan(scene_uid, definitions))

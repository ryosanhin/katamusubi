...
func _call_method(test_root_user, test_child_user) -> void:
	...
	var root_result = test_root_user.call_injected_instance()
	print(root_result)
	if root_result != "call from test root user":
		exit_code += 1<<0

	var child_result = test_child_user.call_injected_instance()
	print(child_result)
	if child_result != "call from test child user":
		exit_code += 1<<1

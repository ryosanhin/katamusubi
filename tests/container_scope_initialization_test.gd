...
func _test_root_scope_without_parent() -> void:
	...
	_check(
			scope.state == Katamusubi.ContainerScope.State.INITIALIZED,
			"ルートスコープが初期化される"
	)
	_check(
			scope._container != null,
			"ルートコンテナが生成される"
	)
	_check(
			scope._container._parent == null,
			"ルートコンテナは親を持たない"
	)
	scope.free()
...
func _test_child_scope_with_valid_parent() -> void:
	...
	_check(
			parent_scope.state == Katamusubi.ContainerScope.State.INITIALIZED,
			"親スコープが先に初期化される"
	)
	_check(
			child_scope.state == Katamusubi.ContainerScope.State.INITIALIZED,
			"子スコープが初期化される"
	)
	_check(
			child_scope._container._parent == parent_scope._container,
			"子コンテナが親コンテナを参照する"
	)
	child_scope.free()
	parent_scope.free()
...
func _test_child_scope_before_parent_is_added() -> void:
	...
	Katamusubi.DEFINITION_LIST.add_scope_definition(definition)

	var child_scope := _create_scope(DELAYED_SCOPE_ID)
	root.add_child(child_scope)
	_check(
		child_scope.state == Katamusubi.ContainerScope.State.NOT_INITIALIZED,
	...
	child_scope.free()
	parent_scope.free()
	Katamusubi.DEFINITION_LIST.remove_scope_definition(DELAYED_SCOPE_ID)
...
func _test_scope_without_definition() -> void:
	...
	_check(
			scope.state == Katamusubi.ContainerScope.State.NOT_INITIALIZED,
			"定義がないスコープは初期化されない"
	)
	_check(
			scope._container == null,
			"定義がないスコープのコンテナは生成されない"
	)
	scope.free()


func _create_scope(id: StringName) -> Katamusubi.ContainerScope:
	var scope := TestScope.new() as Katamusubi.ContainerScope
	scope.scope_id = id
	return scope

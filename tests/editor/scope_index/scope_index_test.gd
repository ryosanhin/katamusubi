...
func _test_scene_replacement_and_duplicates() -> void:
	...
	index.scope_snapshots = [_candidate(&"a", &"old"), _candidate(&"b", &"same")]
	var replacements: Array[ScopeSnapshot] = [
		_candidate(&"a", &"same"), _candidate(&"a", &"same")]
	...
	_runner.assert_equal(index.find_by_id(&"same")[1].scene_uid, &"a", "候補はシーンUIDとIDだけを保持する")
